...
NSString *const CaaSResponseObjectErrorKey = @"CaaSResponseObject";
...
@interface CaaSApiClient ()

@property (readwrite, nonatomic) NSDictionary *HTTPResponseHeaders;

@end

@implementation CaaSApiClient

- (instancetype)init {
    NSString *baseUrl = [[CaaSConfiguration sharedConfig] host];
...
+(NSNumber*) nextRequestId {
    @synchronized(self) {
        ...
        CaaSDebugLog(@"got id %ld", nextId);
        return [NSNumber numberWithLong:nextId];
    }
}

+(NSNumber*) queueRequest {
    NSNumber* requestId = [CaaSApiClient nextRequestId];
    CaaSDebugLog(@"added %@ to request queue", requestId);
...
-(Boolean) executeRequestWithId:(NSNumber*) requestId {
    ...
    if (matchingItems.count == 1) {
        CaaSDebugLog(@"removed request id %@", requestId);
        [queuedRequests removeObject:requestId];
        return YES;
    }
    else {
    ...
    [self.reachabilityManager setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
        ...
        switch (status) {
            case AFNetworkReachabilityStatusUnknown:
                CaaSDebugLog(@"reachability changed to AFNetworkReachabilityStatusUnknown");
                [CaaSApiClient setOfflineState:true];
                break;

            case AFNetworkReachabilityStatusNotReachable:
                CaaSDebugLog(@"reachability changed to AFNetworkReachabilityStatusNotReachable");
                [CaaSApiClient setOfflineState:true];
                break;

            case AFNetworkReachabilityStatusReachableViaWWAN:
                CaaSDebugLog(@"reachability changed to AFNetworkReachabilityStatusReachableViaWWAN");
                [CaaSApiClient setOfflineState:false];
                break;

            case AFNetworkReachabilityStatusReachableViaWiFi:
                CaaSDebugLog(@"reachability changed to AFNetworkReachabilityStatusReachableViaWiFi");
                [CaaSApiClient setOfflineState:false];
                break;
            default:
                break;
                                                               ...
                                                               success:^(AFHTTPRequestOperation *operation, id response) {
                                                                   ...
                                                                   if ([self executeRequestWithId:requestId]) {
                                                                       ...
                                                                       if (operation.responseObject) {
                                                                           ...
                                                                           userInfo[CaaSResponseObjectErrorKey] = operation.responseObject;
                                                                       }
...
-(NSNumber*)  requestWithCompletionBlock: (NSString*) path
    ...
    if ([requestContentType isEqualToString:@"application/json"]) {
        self.requestSerializer = [CaaSJSONRequestSerializer serializer];
    }
    ...
    if ([responseContentType isEqualToString:@"application/json"]) {
        self.responseSerializer = [CaaSJSONResponseSerializer serializer];
    }
    else {
    ...
    [pathParams enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
        [resourcePath replaceCharactersInRange:[resourcePath rangeOfString:[NSString stringWithFormat:@"%@%@%@", @"{", key, @"}"]]
                                    withString:[CaaSApiClient escape:obj]];
    }];
    ...
    if (offlineState) {
        CaaSDebugLog(@"%@ cache forced", resourcePath);
        [request setCachePolicy:NSURLRequestReturnCacheDataDontLoad];
    }
    else if(!hasHeaderParams && [method isEqualToString:@"GET"] && cacheEnabled) {
        CaaSDebugLog(@"%@ cache enabled", resourcePath);
        [request setCachePolicy:NSURLRequestUseProtocolCachePolicy];
    }
    else {
        CaaSDebugLog(@"%@ cache disabled", resourcePath);
    ...
    NSNumber* requestId = [CaaSApiClient queueRequest];
        ...
        for(NSString * key in [queryParams keyEnumerator]){
            ...
            if ([queryParam isKindOfClass:[NSString class]]){
                [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator,
                                          [CaaSApiClient escape:key], [CaaSApiClient escape:[queryParams valueForKey:key]]]];
            }
            else if ([queryParam isKindOfClass:[CaaSQueryParamCollection class]]){
                CaaSQueryParamCollection * coll = (CaaSQueryParamCollection*) queryParam;
                NSArray* values = [coll values];
                NSString* format = [coll format];

                if ([format isEqualToString:@"csv"]) {
                    [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator,
                        [CaaSApiClient escape:key], [NSString stringWithFormat:@"%@", [values componentsJoinedByString:@","]]]];
                    ...
                    for(id obj in values) {
                        [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator,
                            [CaaSApiClient escape:key], [NSString stringWithFormat:@"%@", obj]]];
                        counter += 1;
                    }
                ...
                [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator,
                                          [CaaSApiClient escape:key], [NSString stringWithFormat:@"%@", [queryParams valueForKey:key]]]];
            }
...
- (id) sanitizeForSerialization:(id) object {
    ...
    else if ([object isKindOfClass:[NSString class]] || [object isKindOfClass:[NSNumber class]] || [object isKindOfClass:[CaaSQueryParamCollection class]]) {
        return object;
    }
    ...
    else if ([object isKindOfClass:[CaaSObject class]]) {
