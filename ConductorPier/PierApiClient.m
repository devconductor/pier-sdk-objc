...
NSString *const PierResponseObjectErrorKey = @"PierResponseObject";
...
@interface PierApiClient ()

@property (readwrite, nonatomic) NSDictionary *HTTPResponseHeaders;

@end

@implementation PierApiClient

- (instancetype)init {
    NSString *baseUrl = [[PierConfiguration sharedConfig] host];
...
+(NSNumber*) nextRequestId {
    @synchronized(self) {
        ...
        PierDebugLog(@"got id %ld", nextId);
        return [NSNumber numberWithLong:nextId];
    }
}

+(NSNumber*) queueRequest {
    NSNumber* requestId = [PierApiClient nextRequestId];
    PierDebugLog(@"added %@ to request queue", requestId);
...
-(Boolean) executeRequestWithId:(NSNumber*) requestId {
    ...
    if (matchingItems.count == 1) {
        PierDebugLog(@"removed request id %@", requestId);
        [queuedRequests removeObject:requestId];
        return YES;
    }
    else {
    ...
    [self.reachabilityManager setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
        ...
        switch (status) {
            case AFNetworkReachabilityStatusUnknown:
                PierDebugLog(@"reachability changed to AFNetworkReachabilityStatusUnknown");
                [PierApiClient setOfflineState:true];
                break;

            case AFNetworkReachabilityStatusNotReachable:
                PierDebugLog(@"reachability changed to AFNetworkReachabilityStatusNotReachable");
                [PierApiClient setOfflineState:true];
                break;

            case AFNetworkReachabilityStatusReachableViaWWAN:
                PierDebugLog(@"reachability changed to AFNetworkReachabilityStatusReachableViaWWAN");
                [PierApiClient setOfflineState:false];
                break;

            case AFNetworkReachabilityStatusReachableViaWiFi:
                PierDebugLog(@"reachability changed to AFNetworkReachabilityStatusReachableViaWiFi");
                [PierApiClient setOfflineState:false];
                break;
            default:
                break;
                                                               ...
                                                               success:^(AFHTTPRequestOperation *operation, id response) {
                                                                   ...
                                                                   if ([self executeRequestWithId:requestId]) {
                                                                       ...
                                                                       if (operation.responseObject) {
                                                                           ...
                                                                           userInfo[PierResponseObjectErrorKey] = operation.responseObject;
                                                                       }
...
-(NSNumber*) requestWithPath: (NSString*) path
    ...
    if ([requestContentType isEqualToString:@"application/json"]) {
        self.requestSerializer = [PierJSONRequestSerializer serializer];
    }
    ...
    if ([responseContentType isEqualToString:@"application/json"]) {
        self.responseSerializer = [PierJSONResponseSerializer serializer];
    }
    else {
    ...
    [pathParams enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
        [resourcePath replaceCharactersInRange:[resourcePath rangeOfString:[NSString stringWithFormat:@"%@%@%@", @"{", key, @"}"]]
                                    withString:[PierApiClient escape:obj]];
    }];
    ...
    if (offlineState) {
        PierDebugLog(@"%@ cache forced", resourcePath);
        [request setCachePolicy:NSURLRequestReturnCacheDataDontLoad];
    }
    else if(!hasHeaderParams && [method isEqualToString:@"GET"] && cacheEnabled) {
        PierDebugLog(@"%@ cache enabled", resourcePath);
        [request setCachePolicy:NSURLRequestUseProtocolCachePolicy];
    }
    else {
        PierDebugLog(@"%@ cache disabled", resourcePath);
    ...
    NSNumber* requestId = [PierApiClient queueRequest];
        ...
        for(NSString * key in [queryParams keyEnumerator]){
            ...
            if ([queryParam isKindOfClass:[NSString class]]){
                [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator,
                                          [PierApiClient escape:key], [PierApiClient escape:[queryParams valueForKey:key]]]];
            }
            else if ([queryParam isKindOfClass:[PierQueryParamCollection class]]){
                PierQueryParamCollection * coll = (PierQueryParamCollection*) queryParam;
                NSArray* values = [coll values];
                NSString* format = [coll format];

                if ([format isEqualToString:@"csv"]) {
                    [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator,
                        [PierApiClient escape:key], [NSString stringWithFormat:@"%@", [values componentsJoinedByString:@","]]]];
                    ...
                    for(id obj in values) {
                        [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator,
                            [PierApiClient escape:key], [NSString stringWithFormat:@"%@", obj]]];
                        counter += 1;
                    }
                ...
                [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator,
                                          [PierApiClient escape:key], [NSString stringWithFormat:@"%@", [queryParams valueForKey:key]]]];
            }
...
- (id) sanitizeForSerialization:(id) object {
    ...
    else if ([object isKindOfClass:[NSString class]] || [object isKindOfClass:[NSNumber class]] || [object isKindOfClass:[PierQueryParamCollection class]]) {
        return object;
    }
    ...
    else if ([object isKindOfClass:[PierObject class]]) {
